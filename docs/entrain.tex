...
(a more in-depth discussion of the Thompkins entrainment and mixing material on pp. 56 to 61)
...
integrate the time evolution of the entropy parcel entropy $d\phi/dt$, in terms of the mixing rate $\frac{1}{m}\frac{dm}{dt}$. 
...
to solve four ordinary differential equations, starting with the entropy ($\phi$ or $c_p \log \theta_e$ and the total water mixing ratio $r_T$):


\begin{align}
  \label{eq:odeent}
     \frac{d\,\phi}{dt} &= \lambda \frac{ (h_e - h_c)}{T} \\
     \frac{d\,r_T}{dt} &= \lambda \left (r_{Te} - r_{Tc}) \right )
\end{align}
...
the acceleration for the  updraft speed $w_{vel}$, and the change in height per unit time, which is just $w_{vel}$:
...
sounding.  Next we'll see how this looks in a Jupyter notebook.

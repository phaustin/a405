...
\textit{The following note extends Thompkins Chapter 4 pp. 73-75}

For this course -- understanding the derivation of the droplet growth equation
4.25 is good enough, but the parcel model we are developing as part of entraining\_plume.md needs
something more accurate.   This note shows how to get a more accurate treatment, as derived in
Lohmann Chapter 7 pp. 86-195.
...
\begin{eqnarray}
  ...
  e &=& m_w\, n_v\, R_v\, T
\end{eqnarray}
where $p$  is the total pressure, $v$ is the specific volume (\un{m^3\,kg^{-1}}),
...
$1/\rho = v$) are all independent of position (for example, in a
...
$\rho_v = m_w\,n_v$?
Remember that $p\,v \propto T$, so that from (\ref{eq:state0})
...
It turns out that many people (including Thompkins) write down the diffusion
equation as:
...
\begin{eqnarray}
  \label{balance}
  l_v \frac{dm}{dt} - \frac{dH}{dt} &=& 0 ~\mathrm{~~~~~~~or:} \nonumber\\
  4 \pi  r D l_v ( \rho_{v \infty} - \rho_{v r})&=&  4 \pi r K (T_r
  - T_\infty )
\end{eqnarray}
...
\begin{eqnarray}
  \label{moistad}
  \frac{dT_\infty}{dt}& =& \left [ \frac{ -g}{c_p + l_v \frac{d r_{v \infty}}{dt}}
     \right ] \frac{dz}{dt}~~~~~~~~~~~(\mathrm{moist~adiabat})\nonumber\\
  \frac{dr_{v \infty}}{dt}& =& -\frac{dr_{l}}{dt}= -\frac{4 \pi}{3}
...
Remember also that we can get $\rho_{v \infty}$ from $r_{v \infty}$ through
the definition:

\begin{equation}
  \label{definition}
  r_{v \infty}= \frac{\rho_{v \infty}}{\rho_{d \infty}} = \frac{e/(R_v T_\infty)}{\rho_{d \infty}}
\end{equation}

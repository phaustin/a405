...
(worksheet4_taylor_solution)=
# Worksheet 4: Taylor series solution
...
## Solution: show that {eq}`eq:hydro` is true given {eq}`eq:taylor`:

Start with finding the partial derivatives need for {eq}`eq:taylor`

$$
\begin{aligned} & \frac{\partial p}{\partial T}=R_d \rho \\ 
                & \frac{\partial p}{\partial \rho}=R_d T \\ 
                & \frac{\partial^2 p}{\partial T^2}=0 \\ 
                & \frac{\partial^2 p}{\partial \rho^2}=0 \\
                & \frac{\partial^2 p}{\partial \rho \,\partial T}=R_d
\end{aligned}
$$ (eq:partial)

So now taking $x = \rho$, $y = T$ in {eq}`eq:taylor`, and evaluating the derivatives at
the hydrostatic base state $a=\rho_0$, $b=T_0$, {eq}`eq:taylor` becomes:

+++

$$
\begin{align}
p - p_0 & \approx (\rho- \rho_0)\, R_d T_0 + (T-T_0)\, R_d \rho_0  + (\rho-\rho_0)(T-T_0)R_d
\end{align}
$$ (eq:taylor2)

+++

Finally, divide both sides by $p_0 = R_d \rho_0 T_0$ and use the $\Delta$ notation:


+++

$$
\frac{\Delta p}{p_0}  \approx \frac{\Delta \rho R_d T_0}{ R_d \rho_0 T_0} + 
\frac{\Delta T\,R_d \rho_0}{R_d \rho_0 T_0}
           + \frac{\Delta \rho\, \Delta T\,R_d}{R_d \rho_0 T_0}
$$

+++

Cancelling terms in the numerator and denominator, we get {eq}`eq:hydro`
